...
namespace LectorCvsResultados
{
    public class ConsultasClass
    {
        ...
        /// <param name="fechaNumMaxima">Fecha númerica máxima para realizar la consulta</param>
        /// <returns></returns>
        public static List<AgrupadorConteosTimeSpanDTO> ConsultarConteoSpanTiempoDiaSem(SisResultEntities contexto, string fechaNumMaxima, int caso = 0, int valor = 0)
        {
            string columna;
            string filtro = "";
            switch (caso)
            ...
            string query = string.Format(ConstantesConsulta.QUERY_COUNT_SPANTIEMPOS_DIA_SEM, fechaNumMaxima, columna, filtro);
            DbRawSqlQuery<AgrupadorConteosTimeSpanDTO> data = contexto.Database.SqlQuery<AgrupadorConteosTimeSpanDTO>(query);
            return data.AsEnumerable().ToList();
        }
        ...
        /// <param name="contexto">Instancia del contexto para realizar la consulta</param>
        /// <returns></returns>
        public static List<AgrupadorTotalTabIndexDTO> ConsultarDatosParaDiaSeleccion(int maxListIndex, string fechaFormat, SisResultEntities contexto, string consulta = "")
        {
            ...
            {
                query = string.Format(consulta, fechaFormat, maxListIndex, dayofweek, dt.Day);
            }
            else
            {
                switch (dayofweek)
                {
                    case 1:
                    case 2:
                        query = query = string.Format(ConstantesConsulta.QUERY_SELECCION_ORDENADA_MAS_VALORES_FECHA_PROM, fechaFormat, maxListIndex);
                        break;
                    case 3:
                    case 4:
                    case 5:
                        query = query = string.Format(ConstantesConsulta.QUERY_SELECCION_ORDENADA_MAS_VALORES_DIASEM, fechaFormat, maxListIndex, dayofweek);
                        break;
                    default:
